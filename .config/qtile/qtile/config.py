...
if (is_tool("kitty")):
    terminal = "kitty"
    quakeTerminal = "kitty -T QuakeTerminal"
else:
    terminal = guess_terminal()
    quakeTerminal = guess_terminal()
...
browser_gui = "vivaldi-stable"

screenshot = "flameshot gui"
gifrecord = "peek"
colorpicker = "gpick"

wikibrowser = "qutebrowser"

spawnshortcuts = wikibrowser + " " + dotfiles + "/.shortcuts.html"
...
keys = [EzKey(k[0], *k[1:]) for k in [
    ...
    ("M-S-w", lazy.spawn(browser)),
    
    ("M-S-v", lazy.spawn(editor_gui)),
    ("M-S-e", lazy.spawn(filemanager_gui)),
    ("M-w", lazy.spawn(browser_gui)),
    
    ("<Print>", lazy.spawn(screenshot)),
    ("S-<Print>", lazy.spawn(gifrecord)),
    ("M-p", lazy.spawn(colorpicker)),
    ("M-t", lazy.spawn(terminal + " -- btop")),
    
    ("M-S-t", lazy.spawn("/bin/bash -c '" + tabletscript +"'")),

    # Rofi
    ("M-<grave>", lazy.spawn("rofi -show drun")),
    ("M-S-<grave>", lazy.spawn("rofi -show run")),
    # ("M-C-<grave>", lazy.spawn("rofi -show search -modi search:~/.config/rofi/websearch.py -i -p \"search: \"")),
    ("M-A-<grave>", lazy.spawn("rofi -show power-menu -modi \"power-menu:rofi-power-menu --no-symbols\"")),
    ("M-<Tab>", lazy.spawn("rofi -show windowcd")),
    ("M-S-<Tab>", lazy.spawn("rofi -show window")),
...
floating_layout = layout.Floating(
    float_rules=[
        ...
        *layout.Floating.default_float_rules,
        ...
        Match(title=re.compile(r"Create Shader Node*")), 
        # qtile
...
def set_screen_groups():
    ...
    # screens[2].set_group(groups[1], warp = False)
    # screens[0].set_group(groups[2], warp = False)
    # screens[1].set_group(groups[0], warp = False)
    screens[0].cmd_toggle_group("SYS") # center
    # screens[2].cmd_toggle_group("DEV") # right
    screens[1].cmd_toggle_group("WWW") # left
    # screens[2].cmd_toggle_group("SYS") # center
    # screens[2].cmd_set_wallpaper(home + '/.config/qtile/wallpapers/left.png', )
