...
const TITLE_COLOR = process.env.TITLE_COLOR || '#fbf1c7';
...
Rofi.prototype.updateListView = function updateListView() {
    ...
    this.data.items.forEach(function(item, index) {
        ...
        text += `<span color="${this.config.titleColor}" rise="-35000">` + title + '</span>\n';
        text += '<span size="x-small" rise="0">' + link + '</span>\n';
...
function runRofiSearchWithDeafultConfig() {
    ...
    const theme = $HOME + "/.config/rofi/launchers/type-4/style-2.rasi"
    const env = {
        GOOGLE_ARGS:'["--count", 12]',
        DDG_ARGS:'["-n", 12, "-r", "uk-en"]',
        ROFI_SEARCH: getDefaultSearchMethod()
    };

    const args = ['-modi', 'blocks', '-show', 'blocks', '-eh', 2,
        '-kb-custom-1', 'Control+y', '-theme', theme,
